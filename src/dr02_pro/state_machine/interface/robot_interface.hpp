...
protected:
    ...
    double imu_base_ts_, imu_head_ts_;  // IMU timestamp (milliseconds)

    // Sensor data
    Vec3f omega_head_;  // Angular velocity from IMU (rad/s)
    Vec3f rpy_head_;    // Roll-Pitch-Yaw from IMU (radians)
    Vec3f acc_head_;    // Linear acceleration from IMU (m/s²)
    ...
    rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr imu_data_head_sub_;
...
public:
    ...
    virtual void Start() {
        ...
        imu_data_head_sub_ = node_->create_subscription<sensor_msgs::msg::Imu>(
            "/IMU_DATA_HEAD", 10, std::bind(&RobotInterface::HandleHeadImuData, this, std::placeholders::_1));
    ...
    /**
     * @brief Get current head IMU roll-pitch-yaw data.
     * @return Head IMU roll-pitch-yaw vector.
     */
    virtual Vec3f GetImuHeadRpy() { return rpy_head_; }

    /**
     * @brief Get current head IMU acceleration data.
     * @return Head IMU acceleration vector.
     */
    virtual Vec3f GetImuHeadAcc() { return acc_head_; }

    /**
     * @brief Get current head IMU angular velocity data.
     * @return Head IMU angular velocity vector.
     */
    virtual Vec3f GetImuHeadOmega() { return omega_head_; }
    ...
    /**
     * @brief Get the head IMU timestamp.
     * @return Head IMU timestamp.
     */
    virtual double GetImuHeadTimestamp() { return imu_head_ts_; }
    ...
    /**
     * @brief Handle head IMU messages from /IMU_DATA_HEAD.
     * @param msg Received IMU message.
     */
    virtual void HandleHeadImuData(const sensor_msgs::msg::Imu::SharedPtr msg) {
        rpy_head_ = QuaternionToRpy(*msg);
        acc_head_ << msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z;
        omega_head_ << msg->angular_velocity.x, msg->angular_velocity.y, msg->angular_velocity.z;
        imu_head_ts_ = GetTimestampMs();
    }
